...
In `TwpFastApp`, authentication and authorization are tightly related to `users`, `roles`, and `permissions`.
...
To authorize API end point, you can use any implementation of `AuthService` interface. You can define your own implementation of `AuthService`. But, for start, `TwpFastApp` already has `TokenOAuth2AuthService` and `NoAuthService`.

By default, `TwpFastApp` use `TokenOAuth2AuthService`. You can find the corresponding definition at `main.py`:
...
To authorize a page, you can use any implementation of `MenuService` interface. `TwpFastApp` provide you with default implementation that you can modify later:

...
You can configure `TwpFastApp` using environment variables. For example, you can enable/disable the user interface by passing `0` or `1` to `APP_ENABLE_UI`.

When you run `TwpFastApp` as a monolith, you need to enable every feature flag.
...
At some point, you need to scale your modules independently. You can achieve this without doing any change in `TwpFastApp` codebase. What you need to do is just deploy multiple `TwpFastApp` with different feature flags (configurations).

![Microservices mode](images/fastApp-microservices.png)

Let's take a look at every instance of `TwpFastApp`:
